...
canvasExtModule.directive('apCanvas', function (apImageHelper, $window) {
  return {
    restrict: 'A',
    scope: {
      src: '=',
    ...
    link: function ($scope, element, attrs) {
      ...
      var uploadType = document.getElementById('c-upload-cover') ? 'cover' : 'picture';
      ...
      if ($scope.zoomable) {
        ...
        function setIsMoving(moving, event, position) {
          ...
          if (moving) {
            ...
            canvas.className = "ng-isolate-scope grabbing";
          } else {
            canvas.className = "ng-isolate-scope";
          }
        }
      ...
      var screenSM = 768, screenMD = 992,
          canvasXSwidth, canvasSMwidth, canvasMDwidth,
          canvasXSheight, canvasSMheight, canvasMDheight;
      ...
      function updatefs(zmin) {
        var h1 = 100 - (defaultScale * 100);
        var h2 = 100 - (zmin * 100);

        //
        var origin = 100 - $scope.$parent.priceSlider.value;
        var asp = (origin * 100) / h1;
        var xja = (asp * h2) / 100;
        $scope.$parent.priceSlider.value = Math.floor(100 - xja);

        //
        var origin = 100 - ($scope.scale * 100);
        var asp = (origin * 100) / h1;
        var xja = (asp * h2) / 100;
        // $scope.scale = (100 - xja) / 100;
        setScale((100 - xja) / 100);

        //
        $scope.$parent.priceSlider.floor =  Math.floor(zmin * 100);
      }
      ...
      $scope.$watch(
        ...
        function (newImage, oldImage) {
          ...
          if (newImage) {
            ...
            if (canvas.width == canvasXSwidth) {
              nsize = 1
            } else if (canvas.width == canvasSMwidth)  {
              nsize = 2
            } else {
              nsize = 3
            }
      ...
      $scope.$watch(
        ...
        function (value) {
          ...
          if (value < screenSM) {
            canvas.width = canvasXSwidth;
            canvas.height = canvasXSheight;
          } else if (value < screenMD)  {
            canvas.width = canvasSMwidth;
            canvas.height = canvasSMheight;
          } else {
           canvas.width = canvasMDwidth;
           canvas.height = canvasMDheight;
          }
          ...
          if (image != null) {
            ...
            //     var zmin = canvasSMwidth / image.width ;
            ...
            //     var zmin = canvasMDwidth / image.width ;
            ...
            if (value < screenSM) {
              ...
              if (nsize != 1) {
                ...
                zmin = canvasXSwidth / image.width ;
              ...
              if (nsize != 2) {
                ...
                zmin = canvasSMwidth / image.width ;
              ...
              if (nsize != 3) {
                ...
                zmin = canvasMDwidth / image.width ;
